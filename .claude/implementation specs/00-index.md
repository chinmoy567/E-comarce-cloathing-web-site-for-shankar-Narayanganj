...
Nothing else is deferred. Nine section numbers are not cited verbatim by any spec, because they carry no independent requirement; each is covered through the section it defers to:
...
| `03-payment` §3.11, `04-courier` §4.12, §4.13 | UI-facing narrative diagrams of the status lifecycle. `01-overview`'s closing note designates `07-state-machine` §5.21 as the sole enforced source for these values, and spec 12 implements §5.21 directly — per the `test` skill, tests are written against §5.21, not against these diagrams. |
