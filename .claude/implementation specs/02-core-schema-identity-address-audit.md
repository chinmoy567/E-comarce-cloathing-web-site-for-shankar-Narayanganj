...
- `UNIQUE (phone_number)` — this is the single fact that lets customer-record reuse, the risk-check cache, and the guest per-customer coupon limit all work off one row (§2.9.4, §7.6, §8.8).
...
Seed rows — one per §5.18 matrix row, using the permission keys from §5.16, §8.19, and §13.14. The `manager_tier` column is a verbatim transcription of the §5.18 table; if the two ever disagree, §5.18 wins and this seed is corrected.
...
- `lib/phone.ts` — `normalizeBdPhone(input): string` and `isValidBdPhone(input): boolean`. Every phone value anywhere in the system passes through this before storage or lookup, so record reuse, the risk-check cache key, and the per-customer coupon key all agree on one canonical form (§2.9.4, §7.6, §8.8).
- `lib/password.ts` — `hashPassword` / `verifyPassword` using argon2id (or bcrypt cost ≥ 12). The only place hashing happens (§11.7, §2.1).
- `lib/transaction.ts` — `withTransaction(fn)`. Every later multi-statement atomic operation — order creation, stock decrement, coupon usage, shipment/order cascade (§3, §5.1, §8.25, §5.21.6) — uses this one helper. If the Supabase JS client cannot express a multi-statement transaction, this helper wraps a direct `pg` connection using the same credentials — see assumption 3.
...
1. **Bangladesh phone format.** `02-customer.md` §2.9.3 step 2 requires "Bangladesh phone number format validation" but never defines the format. *Assumption:* the canonical stored form is the 11-digit local format `01[3-9]XXXXXXXX`; inputs in `+880…` or `880…` form are normalized to it before storage or lookup. This must be exactly one helper (`normalizeBdPhone`), because three separate features — record reuse, the risk cache key, and the coupon per-customer key (§2.9.4, §7.6, §8.8) — silently depend on two phone strings comparing equal.
...
3. **Transaction mechanism — DECIDED, not open.** `01-overview.md` §1.1 fixes Supabase as the platform, and order creation, stock decrement, and coupon usage all require genuine multi-statement transactions (§3, §5.1, §8.25). The Supabase JS client does not expose interactive transactions, so **`withTransaction` uses a direct PostgreSQL connection (the `pg` driver) against the same Supabase Postgres instance**, credentials in env; non-transactional reads continue through the Supabase client. This introduces no new database or framework — it is the same PostgreSQL instance §1.1 already mandates, so CLAUDE.md §2 is satisfied. The RPC-function alternative was considered and rejected: it would scatter business logic into SQL functions, splitting the "Express backend is the business-logic authority" rule (CLAUDE.md §3) across two languages.

   **Consequence — two connection paths, one pool cap.** Because the `pg` pool and the Supabase client both consume connections against the same instance, the pool is explicitly sized (`PG_POOL_MAX`, default **10**) and documented in `.env.example` rather than left at the driver default. Supabase projects cap concurrent connections, and an unbounded pool under checkout load exhausts them, which would fail order creation — the one path that must not fail. Every `withTransaction` call also takes a statement timeout so a stuck transaction cannot hold a pooled connection indefinitely. Specs 11–15 assume this helper exists and is used consistently; no slice opens its own connection.
...
5. **Permission count.** The 47 rows above are the §5.18 matrix rows plus §13.14's confirmation that `cms.manage` is the existing key (not a new one). §5.16 says implementation "must not invent additional permission keys beyond this table without updating both" — the extra keys here (`dashboard.view`, `product.*`, `order.view`, etc.) are not inventions: they are §5.18 matrix rows that the shorter table did not need to disambiguate. If a reviewer wants that table to list all 47, that is a PRD edit, not an implementation change.
