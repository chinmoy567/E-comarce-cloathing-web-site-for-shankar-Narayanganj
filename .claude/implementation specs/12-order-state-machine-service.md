...
**COD cancellation — both edges allowed, matching bKash.** COD supports `CONFIRMED → CANCELLED` and `PROCESSING → CANCELLED` exactly as bKash does. All four renderings of the COD paths in §5.21 — the §5.21.3 table, its mermaid diagram, the §5.21.8 "Complete Transition Rules" diagram, and the method-neutral rules in §5.21.7/§5.21.9 — now agree on this.

*Historical note (resolved — no action needed).* These edges were previously the one genuine internal contradiction in the authoritative file: the §5.21.3 COD table omitted `CONFIRMED → CANCELLED` while the §5.21.8 COD diagram omitted `PROCESSING → CANCELLED` — two reciprocal drafting slips, each contradicted by the other rendering *and* by §5.21.7/§5.21.9, which state both edges without qualifying by payment method (the latter lists them as unlabelled rows while explicitly labelling every genuinely method-specific transition). `05-admin-operations.md` §5.1 corroborates independently: its stock-restoration rule covers "any transition into `CANCELLED` … from any state at or after `CONFIRMED`," which would be dead code for COD if COD could not cancel from `CONFIRMED`. **The PRD has since been amended** to add the missing row and the missing edge, so §5.21 is now self-consistent and is the direct reference for this slice. The alternative — honouring each omission literally — would have made a confirmed COD order uncancellable until it reached `PROCESSING` *and* uncancellable again once it did, which no passage asks for and which would strand orders operationally.
...
1. **COD cancellation edges — CLOSED; the PRD has been amended.** This was a documentation defect in §5.21, not an ambiguity about intent: the §5.21.3 COD table omitted `CONFIRMED → CANCELLED` and the §5.21.8 COD diagram omitted `PROCESSING → CANCELLED`, each contradicted by the other rendering and by the method-neutral statements in §5.21.7 and §5.21.9.

   **The recommended amendment has been applied to the PRD:** `CONFIRMED | CANCELLED` was added to §5.21.3's table and its mermaid diagram, and `PROCESSING ──→ CANCELLED` to §5.21.8's COD diagram. All four COD renderings now agree, so **§5.21 is the direct reference for the transition table above** — this spec is no longer a reconciling layer over a conflict, and the table is a plain transcription like every other in this slice. No behavioural change resulted: the table above always allowed both edges. Nothing remains open here.
...
4. **Shipment `CREATED → SHIPPED` permission.** §5.21.9 gives the actor as "Admin / Manager after parcel handover" without naming a permission key, and §5.18 has no "Parcel Handover" row. *Assumption:* `shipment.create` (Yes for Manager) — handover is the completion of the same dispatch action, performed by the same operator, so no separate grant is meaningful. Inventing a key is barred by §5.16, and adding one to §5.18 would be a PRD edit with no behavioural gain.
5. **Manual `PROCESSING → RETURNED`.** §5.21.9 says the transition is "primarily system-triggered… Admin/Manager may also trigger it manually for a documented return outcome not reflected by courier sync," without naming a permission. *Assumption:* `order.update` plus a mandatory reason.
6. **Payment resubmission actor.** §5.21.2 describes the customer resubmitting, and §5.18 has no customer-facing permission row. *Assumption:* `actorType: 'CUSTOMER'` for `REJECTED → PENDING_VERIFICATION`, authorized by order ownership (Order Number + phone, spec 11) rather than by RBAC — consistent with §5.19's rule that customer-facing flows are gated by request-level validation.
7. **`status_sequence` for failure states.** §4.6 requires out-of-order detection but defines no ordering. *Assumption:* failure states inherit the sequence of the state they failed from, so a legitimate `DELIVERY_FAILED → IN_TRANSIT` retry is not misread as stale. This is a mechanism choice, not a new requirement.
8. **Session-local trigger marker.** The guard depends on `set_config('app.transition_ctx', 'on', true)` being transaction-scoped. *Assumption:* all transitional writes go through `withTransaction` (spec 02), which holds a single connection for the transaction — the same prerequisite every other atomic rule in the system already depends on.
