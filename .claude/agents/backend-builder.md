...
1. Read the relevant sections of `requerment.md` for the feature you're building. Do not guess at business rules — they are spelled out in detail (order/payment/shipment state machines in Section 5.21, RBAC permission matrix in Section 5.20, courier abstraction in Section 4.9).
...
- **Payment, order, and shipment status are three independent fields.** Never conflate them or derive one from another implicitly (Section 5.21.11). Every status change must be validated against the explicit transition tables (Section 5.21.1–5.21.9) before touching the DB — reject invalid transitions with a clear error, never silently coerce.
