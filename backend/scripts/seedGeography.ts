...
const DATA_FILE = path.resolve(
  ...
  '../data/geography/bd-adm-cod-v03.json',
);

type Dataset = {
  source: Record<string, string>;
  divisions: Array<{ pcode: string; name: string }>;
  districts: Array<{ pcode: string; name: string; division_pcode: string }>;
  upazilas: Array<{ pcode: string; name: string; district_pcode: string }>;
};
...
export async function seedGeography(connectionString?: string): Promise<GeographySeedResult> {
  const url = connectionString ?? getEnv().DATABASE_URL;
  const dataset = JSON.parse(await readFile(DATA_FILE, 'utf8')) as Dataset;
  ...
  try {
    ...
    for (const division of dataset.divisions) {
      await client.query(
        `INSERT INTO geo_divisions (pcode, name) VALUES ($1, $2)
         ON CONFLICT (pcode) DO UPDATE SET name = EXCLUDED.name, updated_at = now()`,
        [division.pcode, division.name],
      );
    }
    ...
    for (const district of dataset.districts) {
      await client.query(
        `INSERT INTO geo_districts (pcode, name, division_id)
         SELECT $1, $2, d.id FROM geo_divisions d WHERE d.pcode = $3
         ON CONFLICT (pcode) DO UPDATE
           SET name = EXCLUDED.name, division_id = EXCLUDED.division_id, updated_at = now()`,
        [district.pcode, district.name, district.division_pcode],
      );
    }

    for (const upazila of dataset.upazilas) {
      await client.query(
        `INSERT INTO geo_upazilas (pcode, name, district_id, division_id)
         ...
         ON CONFLICT (pcode) DO UPDATE
           SET name = EXCLUDED.name,
               ...
               updated_at = now()`,
        [upazila.pcode, upazila.name, upazila.district_pcode],
      );
    }
    ...
    const counts = await client.query<{ divisions: string; districts: string; upazilas: string }>(
    ...
    if (!row) throw new Error('Geography seed verification returned no counts.');
    ...
    if (
      actual.divisions !== dataset.divisions.length ||
      actual.districts !== dataset.districts.length ||
      actual.upazilas !== dataset.upazilas.length
    ) {
      throw new Error(
        `Geography seed mismatch: expected ${dataset.divisions.length}/${dataset.districts.length}/${dataset.upazilas.length}, ` +
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  seedGeography()
    .then((result) => {
      console.log(
        `geography ${result.version}: ${result.divisions} divisions, ` +
