...
async function main() {
    ...
    try {
        const { rowCount } = await client.query(`INSERT INTO users (role, user_identifier, password_hash, is_system_admin, must_change_password)
       VALUES ('ADMIN', $1, $2, true, true)
       ON CONFLICT ((true)) WHERE is_system_admin DO NOTHING`, [userId, passwordHash]);
        if (rowCount && rowCount > 0) {
            console.log('System admin created.');
            await client.query(`INSERT INTO audit_logs (entity_type, action, actor_type)
