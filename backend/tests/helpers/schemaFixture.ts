...
export function scopedUrl(schema: string): string {
  return `${TEST_DATABASE_URL}?options=${encodeURIComponent(`-c search_path=${schema}`)}`;
}

export async function withAdminClient(sql: string): Promise<void> {
...
export async function connect(schema: string): Promise<pg.Client> {
...
export const SAMPLE_CUSTOMER = {
  ...
  address: {
    division: 'Dhaka',
    district: 'Dhaka',
    areaUnitType: 'THANA' as const,
    areaUnitName: 'Gulshan',
    wardUnitType: 'WARD' as const,
