...
describe.skipIf(!TEST_DATABASE_URL)('catalogue schema constraints (spec 05)', () => {
  ...
  beforeAll(async () => {
    ...
    const category = await db.query<{ id: string }>(
      `INSERT INTO categories (name, slug) VALUES ('Schema Test', 'schema-test') RETURNING id`,
    );
  ...
  it('rejects a direct SQL UPDATE that would leave stock_quantity negative (acceptance 14, test 5)', async () => {
    const variant = await db.query<{ id: string }>(
      `INSERT INTO product_variants (product_id, stock_quantity) VALUES ($1, 0) RETURNING id`,
      [productId],
    );
    const variantId = variant.rows[0]!.id;

    await expect(
      db.query(`UPDATE product_variants SET stock_quantity = -1 WHERE id = $1`, [variantId]),
    ).rejects.toMatchObject({ code: '23514', constraint: 'product_variants_stock_quantity_check' });
  });

  it('rejects an INSERT with a negative stock_quantity (test 5)', async () => {
    await expect(
      db.query(`INSERT INTO product_variants (product_id, stock_quantity) VALUES ($1, -5)`, [productId]),
    ).rejects.toMatchObject({ code: '23514', constraint: 'product_variants_stock_quantity_check' });
  });
  ...
  it('has no column named out_of_stock on products (acceptance 6)', async () => {
    const result = await db.query<{ column_name: string }>(
      `SELECT column_name FROM information_schema.columns
        WHERE table_name = 'products' AND table_schema = $1`,
      [SCHEMA],
    );
  ...
  it('has no column named out_of_stock on product_variants either', async () => {
    const result = await db.query<{ column_name: string }>(
      `SELECT column_name FROM information_schema.columns
        WHERE table_name = 'product_variants' AND table_schema = $1`,
      [SCHEMA],
    );
    const columnNames = result.rows.map((row) => row.column_name);
    expect(columnNames).not.toContain('out_of_stock');
  ...
  it('rejects a compare_at_price below base_price (products_compare_at_price_check)', async () => {
    await expect(
      db.query(
        `INSERT INTO products (category_id, name, slug, base_price, compare_at_price)
         VALUES ($1, 'Bad Compare', 'bad-compare', 100, 50)`,
        [categoryId],
      ),
    ).rejects.toMatchObject({ code: '23514', constraint: 'products_compare_at_price_check' });
  });

  it('rejects a category whose parent_id equals its own id (categories_parent_not_self)', async () => {
    const row = await db.query<{ id: string }>(
      `INSERT INTO categories (name, slug) VALUES ('Self Parent', 'self-parent') RETURNING id`,
    );
    const id = row.rows[0]!.id;
    await expect(
      db.query(`UPDATE categories SET parent_id = $1 WHERE id = $1`, [id]),
    ).rejects.toMatchObject({ code: '23514', constraint: 'categories_parent_not_self' });
  });

  it('rejects a second primary image for the same product (product_images_one_primary_per_product_idx)', async () => {
    await db.query(
      `INSERT INTO product_images (product_id, storage_path, is_primary) VALUES ($1, 'a.jpg', true)`,
      [productId],
    );
    await expect(
      db.query(
        `INSERT INTO product_images (product_id, storage_path, is_primary) VALUES ($1, 'b.jpg', true)`,
        [productId],
      ),
    ).rejects.toMatchObject({ code: '23505', constraint: 'product_images_one_primary_per_product_idx' });
  });
});
