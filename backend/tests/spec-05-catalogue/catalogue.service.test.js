...
describe.skipIf(!TEST_DATABASE_URL)('catalogue service (spec 05)', () => {
    ...
    it.each([
        ...
        ['INACTIVE', true],
        ['INACTIVE', false],
    ])('stores status=%s, isFeatured=%s as an independent combination (test 7)', async (status, isFeatured) => {
        const product = await productsService.createProduct(actor, {
            categoryId,
    ...
    it('resolves concurrent same-name creates to distinct slugs (test 10)', async () => {
        ...
        const attempts = Array.from({ length: 4 }, () => productsService.createProduct(actor, {
            categoryId,
            name,
            basePrice: 100,
            variants: [{ stockQuantity: 1, attributeValueIds: [] }],
    ...
    it('two variants of one product with the same attribute-value combination is rejected with 409 (acceptance 16)', async () => {
        ...
        await expect(productsService.createVariant(actor, product.id, { stockQuantity: 1, attributeValueIds: [value.id] })).rejects.toMatchObject({ status: 409, code: 'VARIANT_COMBINATION_EXISTS' });
    });
    // -------------------------------------------------------------------------
