...
describe.skipIf(!TEST_DATABASE_URL)('inventory.service (05-admin-operations §5.1)', () => {
    ...
    beforeAll(async () => {
        ...
        adminId = (await usersRepository.create({
            role: 'ADMIN',
    ...
    async function createVariants(stockQuantities) {
    ...
    it('exactly one of two concurrent decrements of the last unit succeeds, and stock never goes negative (test 1)', async () => {
        ...
        const attempt = () => withTransaction((client) => inventoryService.decrementStock(client, [{ variantId: variantId, quantity: 1 }], {
    ...
    it('reports the shortfall and writes nothing when stock is insufficient (test 2)', async () => {
        ...
        const result = await withTransaction((client) => inventoryService.decrementStock(client, [{ variantId: variantId, quantity: 3 }], {
    ...
    it('a partially-satisfiable multi-line decrement leaves both quantities unchanged (test 3)', async () => {
        ...
        await expect(withTransaction(async (client) => {
            result = await inventoryService.decrementStock(client, [
    ...
    it('restoreStock round-trips a prior decrement back to the original quantity, with an audit row (test 4, test 13 restore half)', async () => {
        ...
        await withTransaction((client) => inventoryService.decrementStock(client, [{ variantId: variantId, quantity: 3 }], {
            orderId: 'order-roundtrip',
            actorUserId: adminId,
        }));
        expect(await getStock(variantId)).toBe(2);
        await withTransaction((client) => inventoryService.restoreStock(client, [{ variantId: variantId, quantity: 3 }], {
            orderId: 'order-roundtrip',
    ...
    it('two concurrent decrementStock calls on the same two variants in opposite order complete with no SQLSTATE 40P01 (test 15)', async () => {
        ...
        const callA = () => withTransaction((client) => inventoryService.decrementStock(client, [
            { variantId: variantLow, quantity: 1 },
            { variantId: variantHigh, quantity: 1 },
        ], { orderId: 'order-deadlock-a', actorUserId: adminId }));
        const callB = () => withTransaction((client) => inventoryService.decrementStock(client, [
            { variantId: variantHigh, quantity: 1 },
            { variantId: variantLow, quantity: 1 },
        ...
        const rounds = 8;
        const outcomes = await Promise.all(Array.from({ length: rounds }, () => Promise.all([callA(), callB()])));
