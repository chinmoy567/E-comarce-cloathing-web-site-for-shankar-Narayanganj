...
import { isValidOrderTransition, isValidPaymentTransition, isValidShipmentTransition, initialOrderStatus, initialPaymentStatus, initialShipmentStatus, } from '../../src/services/orderStateMachine.js';
describe('Order Status Transitions (§5.21.1, §5.21.3, §5.21.8)', () => {
    ...
    describe('Invalid transitions (§5.21.10)', () => {
        ...
        it('rejects COD_VERIFICATION_PENDING → SHIPPED (COD)', () => {
            const transition = isValidOrderTransition('COD', 'COD_VERIFICATION_PENDING', 'SHIPPED');
            expect(transition).toBeUndefined();
        });
    });
});
...
describe('Shipment Status Transitions (§5.21.4, §5.21.5, §5.21.6, §5.21.8)', () => {
    ...
    it('allows the full happy path: NOT_CREATED → CREATING → CREATED → SHIPPED → IN_TRANSIT → OUT_FOR_DELIVERY → DELIVERED', () => {
        const path = ['NOT_CREATED', 'CREATING', 'CREATED', 'SHIPPED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY', 'DELIVERED'];
