...
describe.skipIf(!TEST_DATABASE_URL)('admin:reset-password script', () => {
  ...
  beforeEach(async () => {
    ...
    const sysAdmin = await db.query<{ id: string }>(
    ...
    const manager = await db.query<{ id: string }>(
      `INSERT INTO users (role, user_identifier, password_hash, must_change_password)
  ...
  it('resets only the is_system_admin row, sets must_change_password, and never touches a Manager', async () => {
    ...
    const { rows } = await db.query<{ password_hash: string; must_change_password: boolean }>(
    ...
    const { rows: mgrRows } = await db.query<{ password_hash: string }>(
      `SELECT password_hash FROM users WHERE id = $1`,
      [managerId],
    );
  ...
  it('revokes every existing refresh_tokens row for the system Admin', async () => {
    await db.query(
      `INSERT INTO refresh_tokens (user_id, token_hash, scope, expires_at)
       VALUES ($1, 'reset-token-hash', 'admin', now() + interval '7 days')`,
      [systemAdminId],
    );
  ...
  it('writes an OUT_OF_BAND_ADMIN_RESET audit row with actor_type SYSTEM (§5.15 rule 10)', async () => {
    ...
    const { rows } = await db.query<{ actor_type: string; entity_id: string }>(
      `SELECT actor_type, entity_id FROM audit_logs WHERE action = 'OUT_OF_BAND_ADMIN_RESET'`,
    );
