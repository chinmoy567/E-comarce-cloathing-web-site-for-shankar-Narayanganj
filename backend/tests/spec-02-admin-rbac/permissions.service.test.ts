...
describe.skipIf(!TEST_DATABASE_URL)('permissions.service resolveEffectivePermissions', () => {
  ...
  it('MANAGER with an ASSIGNED grant gains exactly that key', async () => {
    await withTransaction(async (c) => {
      await c.query(
        `INSERT INTO user_permissions (user_id, permission_key, granted_by) VALUES ($1, 'cms.manage', $2)`,
        [managerId, adminId],
      );
    });
  ...
  it('a stray manager_tier=NO grant row is ignored by resolution (spec 03 test 14)', async () => {
    ...
    await withTransaction(async (c) => {
      await c.query(
        `INSERT INTO user_permissions (user_id, permission_key, granted_by) VALUES ($1, 'user.manager.create', $2)`,
        [managerId, adminId],
      );
    });
  ...
  it('courier.manage and courier.select resolve independently for a default Manager (test 8)', async () => {
    ...
    await withTransaction(async (c) => {
      await c.query(
        `INSERT INTO user_permissions (user_id, permission_key, granted_by) VALUES ($1, 'courier.manage', $2)`,
        [managerId, adminId],
      );
    });
