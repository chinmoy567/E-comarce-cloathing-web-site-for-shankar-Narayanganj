...
describe.skipIf(!TEST_DATABASE_URL)('spec 02 permission catalogue seed', () => {
    ...
    beforeAll(async () => {
        ...
        const { rows } = await db.query('SELECT key, admin_tier, manager_tier, is_administrative FROM permissions');
        seeded = new Map(rows.map((r) => [
            r.key,
    ...
    it('is re-runnable: re-applying the seed adds no duplicate rows', async () => {
        ...
        await db.query(`INSERT INTO permissions (key, label, admin_tier, manager_tier, is_administrative)
       VALUES ('cms.manage', 'CMS Management', 'YES', 'ASSIGNED', false)
       ON CONFLICT (key) DO NOTHING`);
        const { rows } = await db.query('SELECT count(*)::text AS n FROM permissions');
