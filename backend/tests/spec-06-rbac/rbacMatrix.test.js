...
describe.skipIf(!TEST_DATABASE_URL)('RBAC matrix (06-rbac §5.18)', () => {
    ...
    beforeAll(async () => {
        ...
        adminId = (await users.create({ role: 'ADMIN', userIdentifier: 'matrix-admin', passwordHash, mustChangePassword: false })).id;
        managerId = (await users.create({
            role: 'MANAGER',
    ...
    async function meAsAdmin() {
        const session = await loginAsAdmin(app, 'matrix-admin', 'RbacMatrixPass12');
        const res = await session.agent.get('/api/admin/auth/me');
        return res.body.data.permissions;
    }
    it.each(Object.entries(MATRIX))('%s matches the §5.18 matrix (test 1)', async (key, [adminTier, managerTier]) => {
        ...
        await permissionsRepo.revoke(managerId, key);
        ...
        if (managerTier === 'ASSIGNED') {
            await permissionsRepo.grant(managerId, key, adminId);
    ...
    describe('a manager_tier=NO permission is unreachable even if granted directly at the data layer', () => {
        it('user.manager.create stays absent after a direct INSERT (test 14, defence in depth)', async () => {
            ...
            await withTransaction(async (c) => {
                await c.query(`INSERT INTO user_permissions (user_id, permission_key, granted_by)
           VALUES ($1, 'user.manager.create', $2) ON CONFLICT DO NOTHING`, [managerId, adminId]);
            });
            const permissions = await meAsManager();
