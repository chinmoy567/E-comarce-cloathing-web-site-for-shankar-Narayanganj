...
describe.skipIf(!TEST_DATABASE_URL)('managers.service', () => {
    ...
    describe('createManager (test 3, test 9)', () => {
        it('Admin creates a Manager and an audit row records it', async () => {
            const result = await service.createManager({ userId: otherAdminId, role: 'ADMIN' }, { userIdentifier: 'newmgr', password: 'irrelevant-for-this-fixture' });
        ...
        it('Manager cannot create a Manager (§5.14 Rule 2, test 3)', async () => {
            await expect(service.createManager({ userId: managerAId, role: 'MANAGER' }, { userIdentifier: 'illegal-mgr', password: 'irrelevant' })).rejects.toMatchObject({ status: 403 });
        ...
        it('rejects granting a permission the Admin actor does not itself hold — no user_permissions row written (test 5)', async () => {
            ...
            const result = await service.createManager({ userId: otherAdminId, role: 'ADMIN' }, { userIdentifier: 'grant-on-create', password: 'irrelevant', permissions: ['cms.manage'] });
        ...
        it('rejects a YES-tier permission on create with PERMISSION_NOT_ASSIGNABLE', async () => {
            await expect(service.createManager({ userId: otherAdminId, role: 'ADMIN' }, { userIdentifier: 'yes-tier-mgr', password: 'irrelevant', permissions: ['order.confirm'] })).rejects.toMatchObject({ code: 'PERMISSION_NOT_ASSIGNABLE', status: 400 });
        });
        it('rejects a NO-tier permission on create with PERMISSION_NOT_ASSIGNABLE', async () => {
            await expect(service.createManager({ userId: otherAdminId, role: 'ADMIN' }, { userIdentifier: 'no-tier-mgr', password: 'irrelevant', permissions: ['rbac.configure'] })).rejects.toMatchObject({ code: 'PERMISSION_NOT_ASSIGNABLE', status: 400 });
        });
    });
    describe('updateManager (test 2, test 3, test 6)', () => {
        it('Admin updates a Manager and an audit row records previous/new values', async () => {
            const result = await service.updateManager({ userId: otherAdminId, role: 'ADMIN' }, managerAId, { userIdentifier: 'mgrA-renamed' });
        ...
        it('Manager cannot update the Admin account (§5.11, test 2)', async () => {
            await expect(service.updateManager({ userId: managerAId, role: 'MANAGER' }, otherAdminId, {
                userIdentifier: 'hijacked',
            })).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('Manager cannot update another Manager account (§5.11, §5.14 Rule 2, test 3)', async () => {
            await expect(service.updateManager({ userId: managerAId, role: 'MANAGER' }, managerBId, {
        ...
        it('the system Admin id 404s on update, even called by the Admin itself (§5.12.3, test 6)', async () => {
            await expect(service.updateManager({ userId: otherAdminId, role: 'ADMIN' }, systemAdminId, {
        ...
        it('an Admin targeting its own (non-Manager) id 404s — this endpoint only ever operates on Manager rows (§5.12.3)', async () => {
            ...
            await expect(service.updateManager({ userId: otherAdminId, role: 'ADMIN' }, otherAdminId, {
                userIdentifier: 'self-renamed',
            })).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('a Manager cannot modify its own account (§5.15 rule 4)', async () => {
            await expect(service.updateManager({ userId: managerAId, role: 'MANAGER' }, managerAId, {
    ...
    describe('deleteManager (test 2, test 3, test 6, test 9)', () => {
        ...
        it('Manager cannot delete the Admin account (§5.11, test 2)', async () => {
            await expect(service.deleteManager({ userId: managerAId, role: 'MANAGER' }, otherAdminId)).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
            expect(await users.findById(otherAdminId)).not.toBeNull();
        });
        it('Manager cannot delete another Manager (§5.14 Rule 2, test 3)', async () => {
            await expect(service.deleteManager({ userId: managerAId, role: 'MANAGER' }, managerBId)).rejects.toMatchObject({ status: 403 });
        ...
        it('the system Admin id 404s on delete, even called by the Admin itself (§5.12.3, test 6)', async () => {
            await expect(service.deleteManager({ userId: otherAdminId, role: 'ADMIN' }, systemAdminId)).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        ...
        it('an Admin targeting its own (non-Manager) id 404s — this endpoint only ever operates on Manager rows', async () => {
            await expect(service.deleteManager({ userId: otherAdminId, role: 'ADMIN' }, otherAdminId)).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('a Manager cannot delete its own account (§5.15 rule 4)', async () => {
            await expect(service.deleteManager({ userId: managerAId, role: 'MANAGER' }, managerAId)).rejects.toMatchObject({ code: 'CANNOT_MODIFY_SELF', status: 403 });
    ...
    describe('deactivateManager / reactivateManager (test 2, test 3, test 6, test 9)', () => {
        ...
        it('Manager cannot deactivate the Admin account (§5.11, test 2)', async () => {
            await expect(service.deactivateManager({ userId: managerAId, role: 'MANAGER' }, otherAdminId)).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('Manager cannot deactivate another Manager (§5.14 Rule 2, test 3)', async () => {
            await expect(service.deactivateManager({ userId: managerAId, role: 'MANAGER' }, managerBId)).rejects.toMatchObject({ status: 403 });
        });
        it('the system Admin id 404s on deactivate, even called by the Admin itself (§5.12.3, test 6)', async () => {
            await expect(service.deactivateManager({ userId: otherAdminId, role: 'ADMIN' }, systemAdminId)).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
            const untouched = await users.findById(systemAdminId);
    ...
    describe('setManagerPermissions (test 2, 3, 5, 6, 8, 9)', () => {
        ...
        it('Manager cannot set permissions on the Admin account (§5.11, test 2)', async () => {
            await expect(service.setManagerPermissions({ userId: managerAId, role: 'MANAGER' }, otherAdminId, ['cms.manage'])).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('Manager cannot set permissions on another Manager (§5.11, test 3)', async () => {
            await expect(service.setManagerPermissions({ userId: managerAId, role: 'MANAGER' }, managerBId, ['cms.manage'])).rejects.toMatchObject({ status: 403 });
        ...
        it('the system Admin id 404s on permission-set, even called by the Admin itself (§5.12.3, test 6)', async () => {
            await expect(service.setManagerPermissions({ userId: otherAdminId, role: 'ADMIN' }, systemAdminId, ['cms.manage'])).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 });
        });
        it('no self-grant: a Manager cannot set its own permissions (§5.15 rule 4)', async () => {
            await expect(service.setManagerPermissions({ userId: managerAId, role: 'MANAGER' }, managerAId, ['cms.manage'])).rejects.toMatchObject({ code: 'CANNOT_MODIFY_SELF', status: 403 });
        });
        ...
        it('rejects granting a YES-tier key with PERMISSION_NOT_ASSIGNABLE (acceptance 11)', async () => {
            await expect(service.setManagerPermissions({ userId: otherAdminId, role: 'ADMIN' }, managerAId, ['order.confirm'])).rejects.toMatchObject({ code: 'PERMISSION_NOT_ASSIGNABLE', status: 400 });
            expect(await grantsFor(managerAId)).toEqual([]);
        });
        it('rejects granting a NO-tier key with PERMISSION_NOT_ASSIGNABLE (acceptance 12)', async () => {
            await expect(service.setManagerPermissions({ userId: otherAdminId, role: 'ADMIN' }, managerAId, ['rbac.configure'])).rejects.toMatchObject({ code: 'PERMISSION_NOT_ASSIGNABLE', status: 400 });
            expect(await grantsFor(managerAId)).toEqual([]);
        });
        it('courier.manage and courier.select are granted/checked independently (§5.16, test 8)', async () => {
            ...
            await expect(service.setManagerPermissions({ userId: otherAdminId, role: 'ADMIN' }, managerAId, ['courier.select'])).rejects.toMatchObject({ code: 'PERMISSION_NOT_ASSIGNABLE', status: 400 });
