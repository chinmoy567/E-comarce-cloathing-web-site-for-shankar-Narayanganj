  ...
  describe('grant (error table: UNKNOWN_PERMISSION)', () => {
    ...
    it('is idempotent: re-granting adds no second row', async () => {
      ...
      const { count } = await withTransaction(async (c) => {
        const { rows } = await c.query<{ count: string }>(
          'SELECT count(*)::text AS count FROM user_permissions WHERE user_id = $1',
          [managerId],
        );
    ...
    it('rejects a key that is not in the catalogue', async () => {
      await expect(
        ...
        repo.grant(managerId, 'orders.delete.everything', adminId),
      ).rejects.toMatchObject({ code: 'UNKNOWN_PERMISSION', status: 400 });
    });

    it('rejects a grant to a user that does not exist', async () => {
      await expect(
        repo.grant('00000000-0000-0000-0000-000000000000', 'order.cancel', adminId),
      ).rejects.toThrow();
    });
  });
