...
import { ACCOUNT_TYPES, AREA_UNIT_TYPES, PERMISSION_TIERS, WARD_UNIT_TYPES, isAccountType, isAreaUnitType, isWardUnitType, } from '../../src/types/enums.ts';
...
import { ORDER_STATUSES, PAYMENT_METHODS, PAYMENT_STATUSES, SHIPMENT_STATUSES, } from '../../src/types/orderEnums.ts';
/**
...
describe.skipIf(!TEST_DATABASE_URL)('database enums match their TypeScript mirrors', () => {
    ...
    beforeAll(async () => {
        ...
        for (const row of rows)
            actual.set(row.type_name, row.labels);
    }, 60_000);
    afterAll(async () => {
        await client?.end();
...
describe('enum type guards', () => {
    it('accept every documented value and reject anything else', () => {
        ...
        for (const value of AREA_UNIT_TYPES)
            expect(isAreaUnitType(value)).toBe(true);
        for (const value of WARD_UNIT_TYPES)
            expect(isWardUnitType(value)).toBe(true);
