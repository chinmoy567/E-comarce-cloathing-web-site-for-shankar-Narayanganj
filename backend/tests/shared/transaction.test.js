...
describe.skipIf(!TEST_DATABASE_URL)('withTransaction (acceptance 12)', () => {
    ...
    const countCustomers = async () => withTransaction(async (c) => {
        const { rows } = await c.query('SELECT count(*)::text AS n FROM customers');
        return Number(rows[0].n);
    });
    const insertCustomer = async (client, phone) => {
       ...
       ) VALUES ('Rollback Test', $1, 'Dhaka','Dhaka','THANA','Gulshan','WARD','Ward 19','House 1')`, [phone]);
    };
