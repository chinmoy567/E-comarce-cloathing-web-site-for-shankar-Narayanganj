...
describe.skipIf(!TEST_DATABASE_URL)('users repository', () => {
  ...
  describe('create (§2.8, §2.1)', () => {
    ...
    it('rejects a duplicate user identifier with USER_IDENTIFIER_EXISTS', async () => {
      ...
      await expect(
        repo.create({ role: 'MANAGER', userIdentifier: 'mgr01', passwordHash: 'hash' }),
      ).rejects.toMatchObject({ code: 'USER_IDENTIFIER_EXISTS', status: 409 });
    });
    ...
    it('lets the database refuse a malformed role shape (§5.19)', async () => {
      ...
      await expect(
        repo.create({ role: 'CUSTOMER', phoneNumber: '01912345678', passwordHash: 'hash' }),
      ).rejects.toThrow();
    });
