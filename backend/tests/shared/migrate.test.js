 ...
 */
const scopedUrl = () => `${TEST_DATABASE_URL}?options=${encodeURIComponent(`-c search_path=${TEST_SCHEMA}`)}`;
async function withAdminClient(sql) {
...
describe.skipIf(!TEST_DATABASE_URL)('migration runner (spec 01 acceptance 1)', () => {
    beforeEach(async () => {
        await withAdminClient(`DROP SCHEMA IF EXISTS ${TEST_SCHEMA} CASCADE; CREATE SCHEMA ${TEST_SCHEMA};`);
    });
    afterEach(async () => {
    ...
    it('records every applied migration exactly once', async () => {
        ...
        await client.connect();
        try {
            const { rows } = await client.query('SELECT id, count(*)::text AS n FROM schema_migrations GROUP BY id');
