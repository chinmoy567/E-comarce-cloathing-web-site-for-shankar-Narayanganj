...
import { geoEntityExists, listDistrictsByDivision, listDivisions, listUpazilasByDistrict, resolveGeography, } from '../../src/repositories/geography.repository.ts';
...
describeDb('geography repository', () => {
    ...
    it('rejects a mismatched but individually-valid combination', async () => {
        ...
        await expect(resolveGeography({ divisionId: chattogram.id, districtId: cumilla.id, upazilaId: daudkandi.id })).resolves.toMatchObject({ upazila: { name: 'Daudkandi' } });
        // Chattogram -> Dhaka is the task's own example of what must be impossible.
        await expect(resolveGeography({ divisionId: chattogram.id, districtId: dhakaDistrict.id, upazilaId: daudkandi.id })).resolves.toBeUndefined();
        // Upazila real, but not in the named district.
        await expect(resolveGeography({ divisionId: dhaka.id, districtId: dhakaDistrict.id, upazilaId: daudkandi.id })).resolves.toBeUndefined();
    });
    ...
    it('distinguishes an unknown parent from a childless one', async () => {
        ...
        await expect(resolveGeography({ divisionId: unknown, districtId: unknown, upazilaId: unknown })).resolves.toBeUndefined();
    ...
    it('refuses a duplicate pcode and a duplicate name within one parent', async () => {
        ...
        await expect(client.query('INSERT INTO geo_districts (pcode, name, division_id) VALUES ($1, $2, $3)', [
            district.pcode,
            'Some Other Name',
    ...
    it('refuses an upazila whose district/division pair is not real', async () => {
        const { rows: districts } = await client.query('SELECT id, division_id FROM geo_districts ORDER BY pcode LIMIT 2');
        const [a, b] = districts;
        // District from one row, division from another — a pair that exists nowhere.
        await expect(client.query('INSERT INTO geo_upazilas (pcode, name, district_id, division_id) VALUES ($1, $2, $3, $4)', ['BD99999999', 'Nowhere', a.id, b.division_id === a.division_id ? '00000000-0000-4000-8000-000000000000' : b.division_id])).rejects.toMatchObject({ constraint: 'geo_upazilas_district_division_fkey' });
    });
    ...
    it('loads the official counts with no orphans', async () => {
        const { rows } = await client.query(`SELECT (SELECT count(*) FROM geo_divisions) AS divisions,
    ...
    it('accepts a consistent customer address and rejects an inconsistent one', async () => {
        ...
        const cumilla = (await listDistrictsByDivision(chattogram.id)).find((d) => d.name === 'Cumilla');
        const daudkandi = (await listUpazilasByDistrict(cumilla.id)).find((u) => u.name === 'Daudkandi');
        const insert = (phone, divisionId, districtId, upazilaId) => client.query(`INSERT INTO customers
         ...
         VALUES ('T', $1, 'Chattogram', 'Cumilla', 'UPAZILA', 'Daudkandi', 'UNION', 'Union X',
                 'House 12', $2, $3, $4)`, [phone, divisionId, districtId, upazilaId]);
        ...
        await expect(client.query(`INSERT INTO customers
           (full_name, phone_number, division, district, area_unit_type, area_unit_name,
            ward_unit_type, ward_unit_name, detailed_address, division_id, upazila_id)
         VALUES ('T', '01733333333', 'C', 'C', 'UPAZILA', 'D', 'UNION', 'U', 'H', $1, $2)`, [chattogram.id, daudkandi.id])).rejects.toMatchObject({ constraint: 'customers_geography_chain' });
    });
});
