...
const VALID_PNG = Buffer.from('89504e470d0a1a0a0000000d494844520000000100000001080600000' +
    '01f15c4890000000a4944415478da6360000002000155020918a80f0000000049454e44ae426082', 'hex');
...
describe('validateUpload (spec 04 §11.6, test 9, acceptance 11)', () => {
    ...
    it('rejects an SVG even when allowedMimeTypes mistakenly lists it', async () => {
        await expect(validateUpload(SVG_CLAIMING_PNG, { allowedMimeTypes: ['image/svg+xml', 'image/png'], maxBytes: 5 * 1024 * 1024 })).rejects.toMatchObject({ status: 400, code: 'VALIDATION_ERROR' });
    });
    it('rejects HTML content regardless of the declared allowlist', async () => {
        ...
        await expect(validateUpload(html, { allowedMimeTypes: ['text/html', 'image/png'], maxBytes: 5 * 1024 * 1024 })).rejects.toMatchObject({ status: 400, code: 'VALIDATION_ERROR' });
    });
    /**
    ...
    it('rejects a valid PNG when png is not in the caller\'s allowlist', async () => {
        await expect(validateUpload(VALID_PNG, { allowedMimeTypes: ['image/jpeg'], maxBytes: 5 * 1024 * 1024 })).rejects.toMatchObject({ status: 400, code: 'VALIDATION_ERROR' });
    });
