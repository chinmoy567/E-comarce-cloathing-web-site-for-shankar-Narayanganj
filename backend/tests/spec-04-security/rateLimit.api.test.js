...
describe.skipIf(!TEST_DATABASE_URL)('rate limiting (spec 04)', () => {
    ...
    function buildAppWith(overrides) {
        applyTestEnv();
        process.env.DATABASE_URL = scopedUrl(SCHEMA);
    ...
    async function countRows(table) {
        return withTransaction(async (c) => {
            const { rows } = await c.query(`SELECT count(*)::text AS count FROM ${table}`);
    ...
    describe('rejection logging (test 5, §11.2, §5.15 rule 10)', () => {
        it('a 429 appends one audit_logs row with limiter/endpoint/ip in new_value and a hashed identifier, visible via security_events', async () => {
            ...
            const row = await withTransaction(async (c) => {
                const { rows } = await c.query(`SELECT action, actor_type, new_value FROM audit_logs WHERE action = 'rate_limit_rejected' ORDER BY created_at DESC LIMIT 1`);
                return rows[0];
            });
            ...
            const securityEventsRow = await withTransaction(async (c) => {
                const { rows } = await c.query(`SELECT * FROM security_events WHERE action = 'rate_limit_rejected' ORDER BY created_at DESC LIMIT 1`);
                return rows[0];
            });
