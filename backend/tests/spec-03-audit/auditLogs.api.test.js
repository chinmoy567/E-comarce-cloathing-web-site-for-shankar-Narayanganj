...
describe.skipIf(!TEST_DATABASE_URL)('audit logs API', () => {
    ...
    beforeAll(async () => {
        ...
        adminId = (await users.create({ role: 'ADMIN', userIdentifier: 'audit-admin', passwordHash, mustChangePassword: false })).id;
    }, 60_000);
    afterAll(async () => {
        await resetTransactionPool?.();
    ...
    it('shows a matching row after a manager_created action, with actor, action, and timestamp (acceptance 17)', async () => {
        ...
        const row = res.body.data.find((r) => r.entityId === created.body.data.id && r.action === 'manager_created');
    ...
    it('filters by entityType', async () => {
        ...
        const res = await session.agent.get('/api/admin/audit-logs').query({ entityType: 'user' });
        expect(res.status).toBe(200);
        expect(res.body.data.every((r) => r.entityType === 'user')).toBe(true);
    });
