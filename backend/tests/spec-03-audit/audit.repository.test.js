...
describe('audit repository exports (acceptance 11)', () => {
    ...
    it('exposes no export whose name suggests mutation', () => {
        const mutating = Object.keys(auditRepository).filter((name) => /update|delete|remove|purge|destroy|truncate|edit|modify/i.test(name));
...
describe.skipIf(!TEST_DATABASE_URL)('audit repository round-trip', () => {
    ...
    it('lists an entity history newest first', async () => {
        ...
        expect(total).toBe(3);
        expect(items.map((i) => i.newValue.status)).toEqual([
            'DELIVERED',
            'SHIPPED',
            'CONFIRMED',
        ]);
    ...
    it('paginates without losing the total', async () => {
        const { items, total } = await repo.listForEntity('order', '22222222-2222-2222-2222-222222222222', { page: 1, pageSize: 2 });
